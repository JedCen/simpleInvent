<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  ...
  <div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('email') }">
   <input type="email" class="form-control" :placeholder="trans('message.email')" name="email" v-model="form.email" autofocus/>
  ...
  <div class="form-group has-feedback">
   <input type="password" class="form-control" :placeholder="trans('message.retypepassword')" name="password_confirmation" v-model="form.password_confirmation"/>
   <span class="glyphicon glyphicon-lock form-control-feedback"></span>
  </div>
  ...
  <div class="row">
   ...
   <div class="col-xs-8">
    <button type="submit" class="btn btn-primary btn-block btn-flat" :disabled="form.errors.any()"><i v-if="form.submitting" class="fa fa-refresh fa-spin"></i> {{ trans('message.passwordreset') }}</button>
   </div><!-- /.col -->
   <div class="col-xs-2">
   </div><!-- /.col -->
  </div>
  ...
  methods: {
    submit () {
      this.form.post('/password/reset')
       ...
       .catch(error => {
         console.log(this.trans('message.passwordreset') + ':' + error)
       })
    },
    clearErrors (name) {
