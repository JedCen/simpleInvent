  ...
  <div class="row">
   <div class="col-8">
    <div class="checkbox icheck">
     <label>
      <input type="checkbox" name="remember" v-model="form.remember"> {{ trans('message.remember') }}
     </label>
    </div>
   </div>
   <div class="col-4">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      icon: function () {
        if (this.name === 'email') return 'fa-envelope'
        return 'fa-user'
      }
    },
    watch: {
        ...
        this.form.post('/login')
          ...
          .catch(error => {
            ...
            if (error.response.data.errors.email) {
              toastr.warning(error.response.data.errors.email[0]);
            }
            if (error.response.data.errors.password) {
              toastr.warning(error.response.data.errors.password[0]);
            }
          })
      },
      adddomain: function () {
